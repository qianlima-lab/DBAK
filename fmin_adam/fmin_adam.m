function [best, fval, exitflag, output] = fmin_adam(fun, x0, stepSize, beta1, beta2, epsilon, nEpochSize, options , net_n , sample_k , X_train_total, y_train,test_x , test_y , Centers,batch_size)
...
%    fun = @(x, nIter)fun(x);
% end
[~ , precomputedtx_train] = RBF_calcDtw(X_train_total , Centers );
[~ , precomputedtx_test] = RBF_calcDtw(test_x , Centers );
input_n = size(y_train , 1);
y_one_hot_total = zeros(input_n , sample_k);
for i = 1 : sample_k
    y_one_hot_total(:,i) = (y_train == i);
end
N = size(X_train_total , 1);
batch_size = min(batch_size , N);
% - Check that gradients are identical for a given nIter
% if (~bUseAnalyticalGradients)
%    [~, vfGrad0] = fun(x0,X_train_total,y_one_hot_total,precomputedtx_train);
%    [~, vfGrad1] = fun(x0,X_train_total,y_one_hot_total,precomputedtx_train);
%    
...
try
   [fval0, vfCdX0] = fun(x0,X_train_total,y_one_hot_total,precomputedtx_train);
catch mErr
...
TolFunCnt = max(N / batch_size + 1,5);
FunCnt = 0;
TolXCnt =  max(N / batch_size + 1,5);
XCnt = 0;
      
while true
    if mod(optimValues.iteration , 100) == 0
        alpha = x(1:(net_n + 1) * sample_k , :);
        alpha = reshape(alpha , sample_k , net_n + 1);
        beta = x((net_n + 1) * sample_k + 1:end,:);
        fprintf('Iteration:%d\n' , optimValues.iteration);
        disp('Measuring training accuracy...');
        [trainaccuracy ,numRight ,NTEST ] = testRBFN(Centers, beta, alpha,X_train_total,y_train,precomputedtx_train);
        fprintf('Training accuracy: %d / %d, %.1f%%\n', numRight, NTEST, trainaccuracy*100);
        disp('Measuring test accuracy...');
        [accuracy ,numRight ,NTEST  ] = testRBFN(Centers, beta, alpha, test_x,test_y,precomputedtx_test);
        fprintf('Test accuracy: %d / %d, %.1f%%\n', numRight, NTEST, accuracy*100);
        if accuracy>best.testacc
            best.trainacc = trainaccuracy;
            best.testacc = accuracy;
            best.x = x;
   ...
   % 计算批次
    if e_id * batch_size + 1 > N
        shuffle_idx = randperm(N);
        e_id = 0;
    end
        
    select_shuffle_idx = shuffle_idx(e_id * batch_size + 1:min((e_id+1) * batch_size , N));
%    select_shuffle_idx = randperm(N);
%     select_shuffle_idx = select_shuffle_idx(1:batch_size);
    X_train = X_train_total(select_shuffle_idx,:);
    y_one_hot = y_one_hot_total(select_shuffle_idx,:);
    precomputedtw = precomputedtx_train(select_shuffle_idx,:);
    e_id = e_id + 1;
   ...
   % beta 小于0不更新
    tmpbeta = x((net_n + 1) * sample_k + 1:end,:);
    tmpbetastep = vfStep((net_n + 1) * sample_k + 1:end,:);
    
    minusbeta = (tmpbeta(:) - tmpbetastep(:))<1e-3;
    tmpbetastep(minusbeta) = 0;
    vfStep((net_n + 1) * sample_k + 1:end,:) = tmpbetastep;
...
%     costFunc = @(p) ...
%           fun(p,X_train,y_one_hot,precomputedtw);
%     [cost, grad] = costFunc(x);
%     numgrad = computeNumericalGradient(costFunc, x);
%     diff = norm(numgrad-grad)/norm(numgrad+grad);
   
   % - Get next cost and gradient
    
   [fval, optimValues.gradient] = fun(x,X_train,y_one_hot,precomputedtw);
   vfCost(nIter + 1) = fval;
   ...
   bStop = bStop | FA_CallOutputFunctions(options, x, optimValues, 'iter') |  best.testacc == 1;
